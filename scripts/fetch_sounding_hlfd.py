#!/usr/bin/env python3
"""
fetch_sounding_hlfd.py — ICON Hybrid-Level Sounding Fetcher (Refactored for library + service use)
"""
...
# NEW: Main fetching function for library use
def fetch_sounding_direct(
    lat: float,
    lon: float,
    model: str | None = None,
    step: int = 0,
    archive_dir: Path = Path("/tmp/sounding_archive"),
    jobs: int = 20
) -> dict | None:
    """
    Fetch sounding and save original files locally.
    Returns the sounding dict + original filename.
    """
    if model is None:
        model = "icon-d2" if (47 <= lat <= 55 and 5 <= lon <= 15) else "icon-eu"

    cfg = MODEL_CFG[model]
    run_date, run = latest_run(model)

    log.info(f"Fetching {model.upper()} for {lat:.4f}, {lon:.4f} — run {run_date.strftime('%Y%m%d')}/{run}Z +{step}h")

    archive_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    run_str = f"{run_date.strftime('%Y%m%d')}_{run}Z"
    subdir = archive_dir / f"{model}_{run_str}_{timestamp}"
    subdir.mkdir(parents=True, exist_ok=True)

    # Call the original logic but with archive path
    sounding = fetch_sounding_internal(lat, lon, model, run, run_date, step, jobs, subdir)

    if sounding:
        json_path = subdir / f"sounding_{model.upper()}_{run_str}_+{step:03d}h.json"
        json_path.write_text(json.dumps(sounding, indent=2, ensure_ascii=False))
        log.info(f"Archived: {json_path}")
        return sounding, json_path.name

    return None, None


# Internal function (extracted from your original fetch_sounding)
def fetch_sounding_internal(lat, lon, model, run, run_date, step, jobs, output_dir):
    cfg = MODEL_CFG[model]
    n_lev = cfg["n_levels"]
    ...
    name = f"sounding_{model.upper()}_{run_date.strftime('%Y%m%d')}_{run}Z_+{step:03d}h_{lat_s}_{lon_s}.json"
    path = output_dir / name
    path.write_text(json.dumps(sounding, indent=2, ensure_ascii=False))
    log.info(f"  ✓  {name}  ({len(levels)} Level)")
    return sounding
...
def main():
    ...
    ok = 0
    for step in steps:
        if fetch_sounding(args.lat, args.lon, args.model, run, run_date, step, args.jobs, out_dir):
            ok += 1
    log.info(f"Fertig: {ok}/{len(steps)} Profile erstellt.")
