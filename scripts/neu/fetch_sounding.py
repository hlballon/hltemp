...
def model_level_url(model: str, run: str, run_date: datetime, step: int, level: int, param: str) -> str:
    ...
    # ICON-D2 nutzt im Dateinamen kleine Parameternamen
    if model == "icon-d2":
        param_token = param.lower()
    else:
        param_token = param.upper()

    fn = (
        f"{model}_{cfg['scope']}_{cfg['gridtype']}_model-level_"
        f"{date}_{step:03d}_{level}_{param_token}.grib2.bz2"
    )
...
def surface_url(model: str, run: str, run_date: datetime, step: int) -> str:
    ...
    if model == "icon-d2" and param == "2d_ps":
        fn = f"{model}_{cfg['scope']}_{cfg['gridtype']}_single-level_{date}_{step:03d}_2d_ps.grib2.bz2"
        return f"{cfg['url_base']}/{run}/ps/{fn}"
...
def fetch_sounding(lat: float, lon: float, model: str, run: str, run_date: datetime, step: int, jobs: int = 20, output_dir: Path = Path(".")) -> dict | None:
    cfg   = MODEL_CFG[model]
    n_lev = cfg["n_levels"]
