...
const HTTP_SOURCES = [
  ...
  'http://192.168.4.1/readings',
  'http://192.168.88.51/readings',
];
...
// const GEOJSON_FILE_URL = 'https://raw.githubusercontent.com/hlballon/hltemp/refs/heads/main/10548_20250608_000000.geojson.br';
...
// MQTT configuration
const MQTT_BROKER = 'wss://*****.gimmbh.com:8084/mqtt';
...
// Legend Component
const Legend = ({ showCSounding, showIconD2, showGeoJson, showWyoming }) => {
  return (
...
const ScatterPlot = ({
  ...
  const allXValues = [...xValues, ...cSoundingXValues, ...iconD2XValues, ...geoJsonXValues, ...wyomingXValues];
  const validXValues = allXValues.filter(v => v != null);

  const xMin = validXValues.length > 0 ? Math.min(...validXValues) : 0;
  const xMax = validXValues.length > 0 ? Math.max(...validXValues) : 1;
  ...
  const xScale = x =>
    xRange === 0
      ? (width - leftPadding - padding) / 2 + leftPadding
      : leftPadding + ((x - xMin) / xRange) * (width - leftPadding - padding);

  const yScale = y =>
    yRange === 0 ? height / 2 : height - padding - ((y - yMin) / yRange) * (height - 2 * padding);
  ...
  return (
    <View style={styles.chartWrapper}>
      ...
      <Svg width={width} height={height}>
        ...
        {showCSounding &&
          cSoundingData &&
          cSoundingData.filter(point => point[xField] != null).map((point, index) => (
            <Circle
              key={`csounding-${index}`}
              cx={xScale(point[xField])}
              cy={yScale(point[yField])}
        ...
        {showIconD2 &&
          iconD2Data &&
          iconD2Data.filter(point => point[xField] != null).map((point, index) => (
            <Circle
              key={`icond2-${index}`}
              cx={xScale(point[xField])}
              cy={yScale(point[yField])}
        ...
        {showGeoJson &&
          geoJsonData &&
          geoJsonData.filter(point => point[xField] != null).map((point, index) => (
            <Circle
              key={`geojson-${index}`}
              cx={xScale(point[xField])}
              cy={yScale(point[yField])}
        ...
        {showWyoming &&
          wyomingData &&
          wyomingData.filter(point => point[xField] != null).map((point, index) => (
            <Circle
              key={`wyoming-${index}`}
              cx={xScale(point[xField])}
              cy={yScale(point[yField])}
...
const PolarPlot = React.memo(
  ({
    ...
    console.log(
      `PolarPlot: data length=${data.length}, showCSounding=${showCSounding}, cSoundingData length=${cSoundingData ? cSoundingData.length : 0}, showIconD2=${showIconD2}, iconD2Data length=${iconD2Data ? iconD2Data.length : 0}, showGeoJson=${showGeoJson}, geoJsonData length=${geoJsonData ? geoJsonData.length : 0}, showWyoming=${showWyoming}, wyomingData length=${wyomingData ? wyomingData.length : 0}`
    );
          ...
          {[0.25, 0.5, 0.75, 1].map((fraction, index) => {
            ...
            return (
              <React.Fragment key={`ring-${index}`}>
                ...
                <SvgText x={centerX + r + 5} y={centerY} fontSize="8" fontWeight="bold" textAnchor="start">
                  {altitudeLabels[index]} m
                </SvgText>
              </React.Fragment>
            );
          })}
...
export default function App() {
  ...
  const [activeHttpUrl, setActiveHttpUrl] = useState(null);
  ...
  const processData = useCallback((data, elapsedTime) => {
    ...
    const altitude = parseFloat(data.Baro_Alt_m) || 0.0;
  ...
  const tryHttpSources = useCallback(async (elapsedTime) => {
    const prioritizedSources = activeHttpUrl
      ? [activeHttpUrl, ...HTTP_SOURCES.filter(url => url !== activeHttpUrl)]
      : HTTP_SOURCES;

    let lastError = null;

    for (const url of prioritizedSources) {
      try {
        console.log('Trying HTTP source:', url);
        const data = await fetchWithTimeout(url, 3000);
        const isValid = processData(data, elapsedTime);

        if (isValid) {
          if (activeHttpUrl !== url) {
            console.log('Switching active HTTP source to:', url);
            setActiveHttpUrl(url);
          }
          setFetchError(null);
          return true;
        } else {
          lastError = new Error(`Invalid values from ${url}`);
        }
      } catch (err) {
        console.warn('HTTP source failed:', url, err.message);
        lastError = err;
      }
    }

    setActiveHttpUrl(null);
    setFetchError(`All HTTP sources failed${lastError ? ': ' + lastError.message : ''}`);
    return false;
  }, [activeHttpUrl, processData]);
    ...
    try {
      ...
      const processedData = data.features
        ...
        .map(feature => {
          ...
          const direction = props.wdir_deg;
          const speed = props.wspd_kn;
    ...
    try {
      ...
      const data = lines
        .map(line => {
          ...
          return headers.reduce((obj, header, index) => {
            ...
            obj[key] = isNaN(value) ? null : (key === 'speed' ? value * 1.94384 : value);
            return obj;
          }, {});
        })
      ...
      const processedData = data
        .map(point => ({
          ...
          speed: point.speed,
          direction: point.direction != null ? (point.direction + 180) % 360 : null
        }))
        .filter(point =>
          point.altitude != null &&
          point.altitude >= userAltMin &&
          ...
          const data = dataLines
            .map(line => {
              ...
              return {
                ...
                speed: parseFloat(values[2]) * 0.539957,
  ...
  }, [showCSounding, userAltMin, userAltMax, cSoundingData.length]);
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(async () => {
      ...
      try {
        ...
        await tryHttpSources(elapsedTime);
      } catch (error) {
        console.error('HTTP polling error:', error);
        setFetchError(`HTTP polling failed: ${error.message}`);
      } finally {
        isBusy = false;
      }
    }, 1000);
      ...
      <View style={styles.gridRow}>
        <View style={[styles.gridCell, { width: cellWidth }]}>
          <View style={styles.textContent}>
            <Text style={styles.numericTitle}>hlballon Flight Display v:260329</Text>
            ...
            <Text style={styles.dataSourceText}>
              ...
              {dataSource === 'http' && activeHttpUrl ? ` | HTTP: ${activeHttpUrl}` : ''}
            </Text>
            {dataSource === 'mqtt' && (
              <Text style={styles.connectionStatus}>MQTT Status: {connectionStatus}</Text>
            )}
